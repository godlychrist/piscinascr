...
const SocialIcon = ({ name }) => {
    switch (name.toLowerCase()) {
        case 'facebook': // Solo dejamos el case principal
            return (
                    ...
                    <nav className="lg:col-span-4 space-y-8" aria-label="Información de contacto">
                        ...
                        <div className="space-y-6">
                            <div className="flex items-start gap-4">
                                <div className="w-10 h-10 bg-brand-blue/10 rounded-xl flex items-center justify-center text-brand-cyan shrink-0 mt-1" aria-hidden="true">
                                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" /></svg>
                                </div>
                                <div>
                                    <p className="text-gray-500 text-xs font-bold uppercase mb-2">WhatsApp & Llamadas</p>
                                    <a href={`https://wa.me/${CONTACT_INFO.phone.replace(/[\s+]/g, '')}`} target="_blank" rel="noopener noreferrer" className="text-white font-bold text-lg hover:text-brand-cyan transition-colors focus:text-brand-cyan outline-none">{CONTACT_INFO.phone}</a>
                                    <div className="mt-2.5 pt-2.5 border-t border-white/10 grid grid-cols-2 gap-x-4 gap-y-1">
                                        {[
                                            '+506 8318 6818',
                                            '+506 7113 7349',
                                            '+506 6258 3417',
                                            ...
                                            <a
                                                ...
                                                target="_blank"
                                                rel="noopener noreferrer"
                                                className="text-gray-400 text-sm font-medium hover:text-brand-cyan transition-colors focus:text-brand-cyan outline-none"
                                            >
                                                {num}
                                            </a>
                                        ))}
